...
def generate_markdown(
    ...
    *,
    ...
    """Render scan results as a markdown gaps checklist.

    Groups results into blocking / warnings / exceptions / passing, shows
    evidence and fix hints for each gap, and emits a summary table. The
    return value is the full markdown string; the caller writes it.
    """
    ...
    blocking = [
        r for r in results
        if r.get("status") in ("fail", "expired_exception") and r.get("severity") == "red"
    ]
    warnings = [
        r for r in results
        if r.get("status") in ("fail", "expired_exception") and r.get("severity") == "yellow"
    ]
    ...
    if source_name:
        ...
        sources_line = source_name + (f" (v{source_version})" if source_version else "")
    else:
    ...
    lines.append("")
    lines.append(f"**Readiness: {readiness_pct:.0f}%** ({passing_count}/{evaluated} checks passing)")
    lines.append("")
...
def cmd_scan(args):
    ...
    # Markdown gaps checklist — runs in every output mode
