...
async function render(widgetSettings: any): Promise<void> {
  const config = parseConfig(widgetSettings.customSettings?.data);
  ...
  const buildsUrl =
    `${baseUrl}/${encodeURIComponent(project)}/_apis/build/builds` +
  ...
  const builds: Build[] = await fetchJson(buildsUrl, token);
  ...
  const scanResults: Array<{ result: ScanResult; build: Build }> = [];
  for (const build of builds) {
    try {
      const artifactUrl =
        `${baseUrl}/${encodeURIComponent(project)}/_apis/build/builds/${build.id}` +
        `/artifacts?artifactName=readiness-scan&api-version=7.1`;
      const artifact = await fetchJson(artifactUrl, token);
      if (!artifact || !artifact.resource || !artifact.resource.downloadUrl) continue;

      const downloadUrl = artifact.resource.downloadUrl;
      const scanData = await fetchJson(downloadUrl, token);
      scanResults.push({ result: scanData as ScanResult, build });
    } catch {
  ...
  if (previous) {
    ...
    if (delta > 0) { driftText = `▲ +${delta}%`; driftClass = "up"; }
    else if (delta < 0) { driftText = `▼ ${delta}%`; driftClass = "down"; }
  }
...
function parseConfig(data: string | undefined): WidgetConfig {
  if (!data) return { pipelineId: "", branch: "refs/heads/main" };
  try {
